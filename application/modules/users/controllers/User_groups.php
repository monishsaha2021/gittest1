...
class User_groups extends MY_Controller
{
	...
	public function edit_group($id)
	{
      	...
      	foreach($privilege as $key => $value)
        {
			...
			$gp_submodules = $this->ion_auth_model->gp_perm_submodules($value->perm_id);

			//fetch all sub child modules
			$gp_subChildModules = $this->ion_auth_model->gp_perm_submodules($value->perm_id, $value->perm_id);

			//add all data to session
			$gp_data[$value->perm_id]['module'] = $value;
			$gp_data[$value->perm_id]['sub_modules'] = $gp_submodules;

			foreach ($gp_subChildModules as $key2 => $value2) {
				$gp_data[$value->perm_id]['sub_childModules'][$key2] = $value2;
			}
		...
		$data['group'] = $group;
		$data['privileges'] = $gp_data;
		$data['crtPrivilege'] = $currentPrivilege;

		//pr($data['privileges']);exit();
		...
		$data['page_title'] = 'Assign Permissions';
		$this->template->template_one($data); 
		// $this->_render_page("dashboard", $data);
	}
